<!-- Шаблон страницы со способами защиты от CSRF-атаки -->
<template>
  <div>
...
<!-- Скрипт страницы со способами защиты от CSRF-атаки -->
<script>
  ...
  data() {
    return {
      ...
      descriptionTypes: [
        ...
        {
          description: '<b class="strong-text">3. Использование флага Same-Site в сookies</b> позволяет пометить куки для определенного домена. Таким образом проверяется источник запроса, и его не получится выполнить с мошеннического сайта'
        },
        {
          description: '<b class="strong-text">4. Подтверждение пользователя:</b> для особо важных действий, вроде перевода денег или смены пароля, требуйте дополнительное действие от пользователя (ввод капчи или кода подтверждения)'
        },
      ],
    }
  },
...
<!-- Стили страницы со способами защиты от CSRF-атаки -->
<style scoped>
.container {
  display: flex;
